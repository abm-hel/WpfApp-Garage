...
namespace Projet_Garage.Acces
{
 ...
 /// </summary>
 public class A_Entretien_Piece : ADBase
 {
  #region Constructeurs
  public A_Entretien_Piece(string sChaineConnexion)
  	: base(sChaineConnexion)
  { }
  #endregion
  public int Ajouter(int? pieceId, int? entretienId, int? quantite, double? prix, double? tva)
  {
   CreerCommande("AjouterEntretien_Piece");
   int res = 0;
  ...
  {
   CreerCommande("ModifierEntretien_Piece");
   int res = 0;
  ...
  {
   CreerCommande("SelectionnerEntretien_Piece");
   Commande.Parameters.AddWithValue("@Index", Index);
   Commande.Connection.Open();
   SqlDataReader dr = Commande.ExecuteReader();
   List<C_Entretien_Piece> res = new List<C_Entretien_Piece>();
   while (dr.Read())
   {
    C_Entretien_Piece tmp = new C_Entretien_Piece();
  ...
  {
   CreerCommande("SelectionnerEntretien_Piece_ID");
   Commande.Parameters.AddWithValue("@id", id);
   Commande.Connection.Open();
   SqlDataReader dr = Commande.ExecuteReader();
   C_Entretien_Piece res = new C_Entretien_Piece();
   while (dr.Read())
   {
  ...
  {
   CreerCommande("SupprimerEntretien_Piece");
   int res = 0;
   Commande.Parameters.AddWithValue("@id", id);
   Commande.Connection.Open();
